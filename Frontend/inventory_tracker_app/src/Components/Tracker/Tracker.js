...
import {Grid, FormControl, InputLabel, Select, MenuItem, Snackbar, Alert, Tooltip} from '@mui/material'
...
export default function Tracker() {
    ...
    const [failedToReassignWarehouse, setFailedToReassignWarehouse] = useState(false)
    ...
    useEffect(() => {
      axios.get('https://Shopify-Backend-Challenge-F22.codevat.repl.co/get_warehouses')
      .then(({data}) => {
        if(data.length > 0){
          setWarehouses(data)
    ...
    useEffect(() => {
      ...
      if(currWarehouse){
        axios.get(`https://Shopify-Backend-Challenge-F22.codevat.repl.co/get_inventory/${currWarehouse.split(' ').join("_")}`).then(({data}) => {
               if(data.error){
      ...
      */
    }, [currWarehouse, newItemCreated, deletedItems, failedToDeleteItems, reassignedWarehouse])

    const deleteItems = () => {
        //For each item that's selected, make a delete request
        const deleteRequests = []
        for(let item of checkedItems){
            const deleteRequest = axios.delete(`https://Shopify-Backend-Challenge-F22.codevat.repl.co/delete_item`, {
                data: item,
                headers:{
        ...
        Promise.all(deleteRequests).then(() => setDeletedItems(true))
    ...
    const reassignItems = () => {
        ...
        for(let item of checkedItems){
            const reassignRequest = axios.put(`https://Shopify-Backend-Challenge-F22.codevat.repl.co/update_item`, {...item, new_address: warehouseToReassign} ,{
                headers:{
                    "Content-Type": "application/x-www-form-urlencoded"
                }
            })
        ...
        Promise.all(reassignRequests).then(() => {
        ...
        <>
            <Grid container justifyContent='center'>
                <FormControl sx={{width: '25em'}}>
                    <InputLabel sx={{color: 'white'}} id="warehouse-select-label">Warehouse</InputLabel>
                    <Select
                        labelId="warehouse-select-label"
                        id="warehouse-select"
                        value={currWarehouse}
                        label="Warehouse"
                        sx={{backgroundColor: '#052354 !important', svg: {color: 'white'}, color: 'white'}}
                        onChange={e => setCurrWarehouse(e.target.value)}
                    >
                        {warehouses.map(warehouse => <MenuItem value={warehouse.address} key={warehouse.address}>{warehouse.name}</MenuItem>)}
                    </Select>
                </FormControl>
                <Tooltip title="Add New Warehouse">
                    <Grid sx={{margin: '10px 10px'}} item className={styles.iconBtn}>
                        <AddIcon fontSize='large' onClick={() => setCreateWarehouse(true)}/>
                    </Grid>
                </Tooltip>
            </Grid>
            ...
            {checkedItems.length > 0 && 
                <div className={styles.managerialBtns}>
                    <Button onClick={deleteItems} variant="contained" color='error' sx={{marginRight: '1em'}}>Delete</Button>
                    <Button onClick={() => setShowReassignDialog(true)} variant="contained">Assign New Warehouse</Button>
                </div>
            }
            ...
            <Dialog 
                ...
                <Button variant="contained" onClick={() => {
                    ...
                    //Create warehouse
                    axios.put('https://Shopify-Backend-Challenge-F22.codevat.repl.co/create_warehouse', {
            ...
            <Dialog 
                ...
                <DialogContent>
                    ...
                    <FormControl sx={{marginTop:'1em'}} fullWidth>
                        <InputLabel sx={{color: 'white'}} id="reassign-warehouse-select-label">Warehouse</InputLabel>
                        <Select
                        ...
                        >
                            {warehouses.map(warehouse => warehouse.address !== currWarehouse && <MenuItem value={warehouse.address} key={warehouse.address}>{warehouse.name}</MenuItem>)}
                        </Select>
                    </FormControl>
                </DialogContent>
                <DialogActions>
            ...
            >
                <Alert severity='error'>An error occured. Could not create warehouse!</Alert>
            </Snackbar>
            <Snackbar
            ...
            >
                <Alert severity='error'>An error occured. Could not get inventory!</Alert>
            </Snackbar>
            <Snackbar
            ...
            >
                <Alert severity='error'>An error occured. Could not delete item(s)!</Alert>
            </Snackbar>
            <Snackbar
            ...
            >
                <Alert severity='error'>An error occured. Could not reassign item(s) to new warehouse!</Alert>
            </Snackbar>
        </>
    )
}
